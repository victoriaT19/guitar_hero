...
void update_displays(int fd, int score, int errors) {
    ...
    // O hardware da DE2i-150 geralmente usa displays de 7 segmentos com lógica ativa baixa.
    // Isso significa que para acender um segmento, você envia um '0'.
    // Portanto, usamos a negação bit a bit (~) e a máscara `& 0x7F` para cada dígito.

    // Display esquerdo (erros: 1 dígito)
    uint32_t left_display = (~seg7_codes[errors % 10]) & 0x7F;
    ioctl(fd, WR_L_DISPLAY);
    ...
    // Dígito das centenas
    right_display |= (~seg7_codes[(score / 100) % 10]) & 0x7F;
    
    // Dígito das dezenas (deslocado em 7 bits)
    right_display |= ((~seg7_codes[(score / 10) % 10]) & 0x7F) << 7;
    
    // Dígito das unidades (deslocado em 14 bits)
    right_display |= ((~seg7_codes[score % 10]) & 0x7F) << 14;
